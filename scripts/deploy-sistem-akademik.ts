// Script untuk men-deploy sistemAkademik contract
...
async function main() {
  try {
    ...
    console.log(`Deploying sistemAkademik to ${network} network...`);

    // Mendapatkan ContractFactory dari kompilasi Solidity
    const sistemAkademik = await ethers.getContractFactory("sistemAkademik");

    // Inisialisasi deployment
    console.log("Initiating deployment transaction...");
    const sistem = await sistemAkademik.deploy();

    // Tunggu sampai contract di-deploy ke jaringan
    console.log("Waiting for deployment transaction confirmation...");
    await sistem.waitForDeployment();

    // Dapatkan alamat contract
    const contractAddress = await sistem.getAddress();
    console.log(`sistemAkademik deployed successfully to: ${contractAddress}`);
