...
contract CampusCredit is ERC20, ERC20Burnable, Pausable, AccessControl {
    ...
    function pause() public {
        ...
        require(hasRole(PAUSER_ROLE,msg.sender), "Only role Pauser!");
        _pause();
    }
    ...
    function mint(address to, uint256 amount) public {
        ...
        require(hasRole(MINTER_ROLE,msg.sender), "Hanya role mint!");
        require(amount <= 1000000);
        _mint(to,amount);
    }

    function Min() public view returns (address, bool) {
        return (msg.sender, hasRole(MINTER_ROLE, msg.sender));
    }

    function Pau() public view returns (address, bool) {
        return (msg.sender, hasRole(PAUSER_ROLE, msg.sender));
    }
    ...
    function transferWithLimit(address to, uint256 amount) public {
        ...
        require(amount <= balanceOf(msg.sender), "Melebihi saldo!");
        require(spentToday[msg.sender] + amount < dailySpendingLimit[msg.sender], "Melebihi batas harian!");
        require(isMerchant[to], "Merchant salah!");
    ...
    function transferWithCashback(address merchant, uint256 amount) public {
        ...
        require(amount <= balanceOf(msg.sender), "Melebihi saldo!");
        require(spentToday[msg.sender] + amount < dailySpendingLimit[msg.sender], "Melebihi batas harian!");
        require(isMerchant[merchant], "Merchant salah!");

        _update(msg.sender, merchant, amount);
        _mint(msg.sender, amount * 2 / 100);
    }
}